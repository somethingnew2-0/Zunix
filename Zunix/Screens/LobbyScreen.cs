...
namespace Zunix.Screens
{
    ...
    public class LobbyScreen : MenuScreen, IDisposable
    {
        ...
        const string instructions =
            "Press X to mark/unmark ready, LB/RB to toggle color, LT/RT to toggle tank";
        ...
        public override void LoadContent()
        {
            ...
            world = new World(Device, ScreenManager.Content,
                networkSession);
            ...
            bool otherScreenHasFocus, bool coveredByOtherScreen)
        {
            ...
            if ((world != null) && !otherScreenHasFocus && !coveredByOtherScreen)
            {
                if (world.GameWon)
                {
                    ...
                    world.Dispose();
                    world = null;
                    // make sure that all of the tanks have cleaned up
                ...
                {
                    world.Update((float)elapsedTime.ElapsedGameTime.TotalSeconds, true);
                }
            }
            ...
            if (otherScreenHasFocus == false)
            {
                ...
                {
                    OldGameplayScreen gameplayScreen =
                        new OldGameplayScreen(networkSession, world);
        ...
        public override void HandleInput(InputState input)
        {
            ...
            if ((networkSession != null) && (networkSession.LocalGamers.Count > 0))
            {
                ...
                if (playerData != null)
                {
                    ...
                    //if (input.TankColorChangeUp)
                    //{
                    //    playerData.TankColor = Tank.GetNextUniqueColorIndex(
                    //        playerData.TankColor, networkSession);
                    //    playerDataChanged = true;
                    //}
                    //else if (input.TankColorChangeDown)
                    //{
                    //    playerData.TankColor = Tank.GetPreviousUniqueColorIndex(
                    //        playerData.TankColor, networkSession);
                    //    playerDataChanged = true;
                    //}
                    //if (input.TankModelChangeUp)
                    //{
                    //    playerData.TankVariation =
                    //        (byte)((playerData.TankVariation + 1) % 4);
                    //    playerDataChanged = true;
                    //}
                    //else if (input.TankModelChangeDown)
                    //{
                    //    if (playerData.TankVariation == 0)
                    //    {
                    //        playerData.TankVariation = 3;
                    //    }
                    //    else
                    //    {
                    //        playerData.TankVariation--;
        ...
        public override void Draw(GameTime gameTime)
        {
            ...
            if (networkSession != null)
            {
                ...
                {
                    world.DrawPlayerData((float)elapsedTime.TotalGameTime.TotalSeconds,
        ...
        void networkSession_GamerJoined(object sender, GamerJoinedEventArgs e)
        {
            ...
            for (int i = 0; i < networkSession.AllGamers.Count; i++)
            {
                if (networkSession.AllGamers[i] == e.Gamer)
                {
                    ...
                    //playerData.TankVariation = (byte)(i % 4);
                    //playerData.TankColor = (byte)i;
                }
            }
